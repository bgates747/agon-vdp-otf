...
typedef struct tag_Pingo3dControl {
    ...
    void initialize(VDUStreamProcessor& processor, uint16_t width, uint16_t height) {
        ...
        if (!m_frame) {
            printf("initialize: failed to allocate %u bytes for frame\n", size);
            show_free_ram();
        }
        ...
        if (!m_zeta) {
            printf("initialize: failed to allocate %u bytes for zeta\n", size);
            show_free_ram();
        }
    ...
    void handle_subcommand(VDUStreamProcessor& processor, uint8_t subcmd) {
        //printf("P3D: handle_subcommand(%hu)\n", subcmd);
        m_proc = &processor;
        switch (subcmd) {
    ...
    void define_mesh_vertices() {
        ...
        if (n > 0) {
            ...
            if (!pos) {
                printf("define_mesh_vertices: failed to allocate %u bytes\n", size);
                show_free_ram();
            }
    ...
    void set_mesh_vertex_indexes() {
        ...
        if (n > 0) {
            ...
            if (!idx) {
                printf("set_mesh_vertex_indexes: failed to allocate %u bytes\n", size);
                show_free_ram();
            }
            printf("Reading %u vertex indexes\n", n);
            for (uint32_t i = 0; i < n; i++) {
                uint16_t index = m_proc->readWord_t();
                if (idx) {
                    *idx++ = index;
                }
                if (!(i & 0x1F)) printf("%u %hu\n", i, index);
            }
            printf("\n");
        }
    }
    ...
    void define_mesh_texture_coordinates() {
        ...
        if (n > 0) {
            ...
            printf("Reading %u texture coordinates\n", n);
            for (uint32_t i = 0; i < n; i++) {
    ...
    void set_texture_coordinate_indexes() {
        ...
        if (n > 0) {
            ...
            if (!idx) {
                printf("set_texture_coordinate_indexes: failed to allocate %u bytes\n", size);
                show_free_ram();
            }
            printf("Reading %u texture coordinate indexes\n", n);
            for (uint32_t i = 0; i < n; i++) {
                uint16_t index = m_proc->readWord_t();
    ...
    void define_mesh_normals() {
        ...
        if (n > 0) {
            ...
            if (!norm) {
                printf("define_mesh_normals: failed to allocate %u bytes\n", size);
                show_free_ram();
            }
            printf("Reading %u normals\n", n);
            for (uint32_t i = 0; i < n; i++) {
                uint16_t x = m_proc->readWord_t();
                uint16_t y = m_proc->readWord_t();
                uint16_t z = m_proc->readWord_t();
    ...
    void set_normal_indexes() {
        ...
        if (n > 0) {
            ...
            if (!idx) {
                printf("set_normal_indexes: failed to allocate %u bytes\n", size);
                show_free_ram();
            }
            printf("Reading %u normals indexes\n", n);
            for (uint32_t i = 0; i < n; i++) {
                uint16_t index = m_proc->readWord_t();
                if (idx && (i < mesh->indexes_count)) {
                    *idx++ = index;
        ...
        if (object && mesh && bmid) {
            ...
            if (stored_bitmap) {
                ...
                if (bitmap) {
                    ...
                    printf("Texture data:  %02hX %02hX %02hX %02hX\n", pix->r, pix->g, pix->b, pix->a);
                }
            }
        }
    }
...
extern "C" {
    ...
    void show_pixel(float x, float y, uint8_t a, uint8_t b, uint8_t g, uint8_t r) {
        printf("%f %f %02hX %02hX %02hX %02hX\n", x, y, a, b, g, r);
    }
#endif
