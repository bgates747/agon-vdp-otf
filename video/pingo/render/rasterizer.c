...
int rasterizer_draw_pixel_perfect(Vec2i off, Renderer *r, Texture *src) {
    ...
    // Clamp max/min values to destination buffer
...
int rasterizer_draw_transformed(Mat4 t, Renderer *r, Texture *src) {
    ...
    // Iterate over the pixels of the axis-aligned bounding box (AABB) which contain the source frame
    for (int y = minY; y <= maxY; y++) {
        for (int x = minX; x <= maxX; x++) {
            ...
            // Check if transformed coords are inside the frame
            if (srcPosF.x < 0 || srcPosF.y < 0 || srcPosF.x >= src->size.x || srcPosF.y >= src->size.y)
                continue;
            Pixel color = texture_read(src, srcPosI);
#endif
            ...
            // Check if transformed coords are inside the frame
            if (srcPosF.x < 0 || srcPosF.y < 0 || srcPosF.x >= src->size.x - 1 || srcPosF.y >= src->size.y - 1)
                continue;
            ...
            if (srcPosF1.x < 0 && srcPosF2.x < 0 || srcPosF1.y < 0 && srcPosF2.y < 0 || srcPosF1.x > src->size.x && srcPosF2.x > src->size.x || srcPosF1.y > src->size.y && srcPosF2.y > src->size.y)
                continue;
            ...
            Pixel color = (Pixel){(p1 + p2 + p3 + p4) / 4};
#endif
            ...
            if (srcPosF1.x < 0 && srcPosF2.x < 0 && srcPosF3.x < 0 && srcPosF4.x < 0 || srcPosF1.y < 0 && srcPosF2.y < 0 && srcPosF3.y < 0 && srcPosF4.y < 0 || srcPosF1.x > src->size.x && srcPosF2.x > src->size.x && srcPosF3.x > src->size.x && srcPosF4.x > src->size.x || srcPosF1.y > src->size.y && srcPosF2.y > src->size.y && srcPosF3.y > src->size.y && srcPosF4.y > src->size.y)
                continue;
